...
- 翻譯機制：引擎會對每個宣告 `data = true` 的 addon 自動載入
  ...
  `~/repo/moddings/tome4/projects/t-engine4/engines/te4-1.7.6/engine/Module.lua`）。
...
輸出到 `~/repo/moddings/tome4/derived/tome4-ch/tome-<name>-zh/`（`<name>` 見任務指派）：
...
`~/repo/moddings/tome4/projects/t-engine4/modules/tome/data/locales/zh_hant.lua`（43k 行，**用 grep 查，別整檔讀**）。
...
lua5.1 ~/repo/moddings/tome4/derived/tome4-ch/_tools/check_locale.lua \
    ~/repo/moddings/tome4/derived/tome4-ch/tome-<name>-zh/data/locales/zh_hant.lua \
    ~/repo/moddings/tome4/derived/tome4-ch/_reference/orig/<name>/
```
