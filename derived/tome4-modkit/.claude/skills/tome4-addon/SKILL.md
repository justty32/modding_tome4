...
工作區：`~/repo/moddings/tome4/derived/tome4-modkit/`（以下路徑皆相對於此）
...
2. **不要憑記憶寫 ToME 的 API。** 先讀 `knowledge/`，沒有的話去 `~/repo/moddings/tome4/projects/t-engine4/` grep 原始碼，讀完把結論**附行號**補進 `knowledge/`。
3. **`~/repo/moddings/tome4/analysis/t-engine/` 不是權威**，只是索引。
4. **唯讀區**：`projects/t-engine4/`、`~/.steam/.../TalesMajEyal/`、`~/repo/moddings/tome4/external/orig/`。
...
- `~/repo/moddings/tome4/external/orig/{arcanum,nullpack,midnight}/` — 實裝過的第三方職業包（唯讀）
