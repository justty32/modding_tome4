...
| 項目 | 狀態 | 下一步 |
|------|------|--------|
| **符文盤（Rune Board）自訂 UI 面板** | 排隊（使用者明確要求，但說先不做） | 先查 ToME 的 `engine.ui.Dialog` 與面板掛載方式，寫 `knowledge/ui-dialogs.md` |
...
| i18n：技能與描述接進 `~/repo/moddings/tome4/derived/tome4-ch` 漢化管線 | 冷凍 | 等內容穩定 |
...
- `mods/tome-runewright/data/lib/resonance.lua` 是**純函數模組**，不碰 actor 狀態。
