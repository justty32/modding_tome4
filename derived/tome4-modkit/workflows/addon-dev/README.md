...
1. [knowledge/](../../knowledge/) — 本專案已複驗過的引擎事實（附 `檔案:行號`）。
2. 沒有的話，去 `~/repo/moddings/tome4/projects/t-engine4/` grep 原始碼，讀完把結論補進 `knowledge/`。
3. 真實範例：`~/repo/moddings/tome4/external/orig/{arcanum,nullpack,midnight}/` 是實裝過的職業包。

`~/repo/moddings/tome4/analysis/t-engine/` 只當索引用，**不是權威**。
...
改用 `tools/run.sh`。兩件事都在 [knowledge/real-machine.md](../../knowledge/real-machine.md)。
...
細節與座標速查見 [knowledge/playtesting.md](../../knowledge/playtesting.md)。
