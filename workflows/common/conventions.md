...
```text
vendor/t-engine4 原始碼 / 實測  >  knowledge/  >  sub_proj/analysis/t-engine（索引，非權威）  >  一般文檔
```

規則：

1. 上層與下層衝突時，以上層為準並修正下層。
2. `sub_proj/analysis/t-engine/` 只是索引；任何 API 結論都要回 `vendor/t-engine4/` 原始碼複驗。
3. `knowledge/` 是本 repo 的引擎真相層——每條結論都附行號，比 `analysis/` 可信。
