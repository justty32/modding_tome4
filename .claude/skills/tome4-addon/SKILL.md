...
2. **不要憑記憶寫 ToME 的 API。** 先讀 `docs/knowledge/`，沒有的話去 `~/repo/moddings/tome4/vendor/t-engine4/` grep 原始碼，讀完把結論**附行號**補進 `docs/knowledge/`。
3. **`~/repo/moddings/tome4/docs/analysis/` 不是權威**，只是索引。
...
8. **使用者不能從 Steam 開遊戲**——引擎自身的工坊同步回呼會 SIGSEGV，與 addon 無關。
   用 `tools/run.sh`（帶 `--no-steam`）。詳見 `docs/knowledge/real-machine.md`。
...
探測手法全集與原理：`docs/knowledge/playtesting-parts/03-state-probes.md`。
...
其餘坑見 `docs/knowledge/README.md` 的「這個引擎很愛靜默失敗」總表。
...
範例：`self_mods/tome-runewright/hooks/load.lua` 結尾。

## 知識層

| 檔案 | 內容 |
|---|---|
| `docs/knowledge/README.md` | 索引；先讀這份 |
| `docs/knowledge/addon-loading.md` | init.lua 欄位、私有掛載、版本、weight |
| `docs/knowledge/class-and-talents.md` | Birther、newTalent、自訂資源、i18n |
| `docs/knowledge/visuals-and-sounds.md` | 粒子、音效、圖示 |
| `docs/knowledge/headless-testing.md` | Xvfb 啟動、為何跳不過主選單、預裝 addon 地雷 |
| `docs/knowledge/playtesting.md` | 實機遊玩、座標速查、TEMP-DEBUG |
...
- `self_mods/tome-runewright/` — 本工具鏈做的職業
