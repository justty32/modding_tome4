# self_mods/dist —— 自製 addon 成品區

本工作區產出的**可對外使用產物**統一放這裡,等待被使用/發佈。這裡只放產物,不放原始碼(addon 原始碼在同層的 `self_mods/tome-*/`)。
...
- 每個成品一個子資料夾,名字含版本:`<名稱>-<版本>/`(例:`tome-runewright-0.1.0/`),內含 `.teaa` 與 `SOURCE.md`(記錄由 `self_mods/` 下哪個 addon、什麼日期、用什麼指令建出;addon 的 `version` 相容性要求見根 `AGENTS.md`「release/package 注意事項」)。
- **與隔壁 `self_mods/build/` 的分工**:`build/` 是開發迴圈的暫存建置產物(build→deploy→測試,隨時被覆蓋,不保證與源碼同步);`dist/` 是確認過、帶版本、可交付的成品。要「找做好的 addon 去部署」一律以這裡為準,這裡沒有就代表尚未發佈。
