...
- **實機試玩女巫森林 `tome-witchwood`**（三 agent 平行產出的新地區）。
  無頭測試證明：三隻怪與 zone 註冊成功、支線任務 `witchwood-curse` 流程通過模擬測試、
  美術已審核。**但沒有人真的走進去玩過。**
  要看的是：瑞文谷（Derth）西北方 (23,15) 大地圖入口有沒有出現、進得去嗎、
  守根人葛薇有沒有生成在走得到的位置、任務能不能完整走完、地形貼圖鋪起來好不好看。
  尚未 deploy，要玩得先跑 `tools/deploy.sh witchwood`。
...
- **盧恩術士的技能特效殘留**（2026-08-01 你回報，當時說先不管）。
  某些技能的特效在技能結束、角色離開後仍留在原地。已知線索：`arcane_power` 這個
  「永不停止」的粒子出現在自製 addon 的用字裡。女巫的同類 bug 已修並記進 knowledge，
  照那份判定方法查即可——這條**不需要你做什麼，是給下個 session 的待辦**。
