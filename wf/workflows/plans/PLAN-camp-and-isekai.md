...
見 runeisles 的踩坑史）。以下是可直接照做的建置藍圖。研究來源見 docs/knowledge/{worldmap-and-zones,
quests-and-lore,npc-and-chats,companions-and-party}.md 與 docs/analysis/tutorial/{10,11}-base-camp-*.md。
...
2. **回程**：召回天賦 toggle——在 camp 就 changeLevel 回 `self.camp_return`（召回前存的
   ...
   - 進階（更貼「大地圖據點」語意）：改用 runeisles 的 `Entity:loadList`+`MapGeneratorStatic:subgenRegister`
     ...
     回 "camp+camp"。但這會碰到 wild_x/wild_y 單組全域 bug（docs/knowledge/worldmap-and-zones.md §5），
...
3. **公會 + 討伐任務**：公會 NPC（`can_talk="orario+guild"`，焊在 hub 地圖 defineTile 第4參），
   ...
   （細節與行號見 docs/knowledge/quests-and-lore.md 與過夜研究報告。）
...
- 粒子研究（docs/knowledge/particles.md）＋ 上面兩個系統的研究都已落檔。
